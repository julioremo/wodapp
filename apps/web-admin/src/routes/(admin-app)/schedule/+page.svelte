<script lang="ts">
  ...
  import { addWeeks, startOfWeek, endOfWeek, format, getWeek } from "date-fns";
  import WeeklyGrid from "$lib/components/scheduler/WeeklyGrid.svelte";
  import ScheduleCard from "$lib/components/scheduler/ScheduleCard.svelte";
  import EditClassForm from "$lib/components/scheduler/EditClassForm.svelte";
...
<div class="flex flex-col h-full gap-5 overflow-hidden">
  ...
  <div class="flex-1 min-h-0 relative">
    <WeeklyGrid classes={thisWeekClasses}>
      {#snippet children({ item, style })}
        <ScheduleCard session={item} {style} class="absolute left-1 right-1 shadow-sm">
