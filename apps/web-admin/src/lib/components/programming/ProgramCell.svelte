...
function handleSubmit({ action, cancel }) {
  ...
  if (!isDelete) {
    blocks.forEach((b) => {
      ...
      b.title = firstLine.startsWith("#")
        ? firstLine.replace(/^#+\s*/, "").trim()
        : `${b.workout_type} Block`;
    });
...
<div
  ...
  <form
    ...
    class="flex flex-col min-h-[320px] px-2 py-2 gap-3 selection:bg-yellow">
    ...
    <div
      class="flex justify-between items-center pb-1.5 border-b-2 border-primary border-dashed w-full gap-3">
      <input
        name="title"
      ...
      <div
        class="text-[10px] text-muted-foreground/60 flex items-center leading-none gap-1 min-w-4 justify-end">
        {#if !isScheduled}
    ...
    <div class="flex flex-col gap-1 flex-1">
      {#each blocks as block, i}
        <div
          class="group/block relative flex gap-2 p-1 py-2 -mx-1 rounded hover:bg-card/70 transition-colors">
          <div class="flex flex-col w-full gap-1 py-1">
            <div class="flex items-center justify-between mt-1">
              <div class="w-full flex justify-between">
                ...
                <div
                  class="flex items-center text-[10px] text-muted-foreground bg-stone-100 px-1.5 py-0.5 rounded focus-within:bg-background focus-within:ring-1 focus-within:ring-border focus-within:text-foreground hover:bg-background/80 transition-all cursor-text">
                  <input
                    type="number"
                    placeholder="0"
                    min="0"
            ...
            <div
              ...
              class="text-md font-mono leading-relaxed focus:outline-none min-h-[24px] empty:before:content-[attr(data-placeholder)] empty:before:text-muted-foreground/25"></div>
          </div>
        </div>
      {/each}
      <button
