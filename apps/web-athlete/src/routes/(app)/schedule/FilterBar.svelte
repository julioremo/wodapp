<script lang="ts">
  ...
  let isTimeActive = $derived(
    timeRange[0] > minBound || timeRange[1] < maxBound,
  );
  ...
  let availableLessonTypes = $derived.by(() => {
  ...
  let shouldPromoteLessonTypes = $derived(availableLessonTypes.length <= 6);
  ...
  $effect(() => {
    if (selectedCoaches.length > 0) {
      const valid = selectedTypes.filter((t) =>
        availableLessonTypes.includes(t),
      );
...
{#snippet classTypesContent()}
  <div class="flex flex-wrap gap-2 justify-start">
    {#each availableLessonTypes as type (type)}
...
>
  ...
  {#if shouldPromoteLessonTypes}
    ...
    {/each}
  {:else if availableLessonTypes.length > 0}
    {@render FilterBlock({
