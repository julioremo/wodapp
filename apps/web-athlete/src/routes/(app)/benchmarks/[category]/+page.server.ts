...
export const load: PageServerLoad = async ({ params, locals: { supabase } }) => {
  ...
  records?.forEach((record) => {
    ...
    const compareValue = mov.measurement_type === "weight" ? record.estimated_1rm : record.score;

    // TS FIX: Skip this record entirely if the required value is null
    ...
    if (currentPr === undefined) {
      ...
      if (isTime && compareValue < currentPr) {
        prMap.set(mov.id, compareValue);
      } else if (!isTime && compareValue > currentPr) {
        prMap.set(mov.id, compareValue);
      }
    }
  });
