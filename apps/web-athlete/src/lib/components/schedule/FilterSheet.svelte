<script lang="ts">
  ...
  let { 
      children, 
      filterData = { types: [], bounds: { min: 360, max: 1320 } } 
  } = $props();
    ...
    <div class="grid gap-8 py-6">
        ...
        <div class="space-y-3">
            <div class="flex justify-between items-center">
                ...
                {#if hiddenTypes.size > 0}
                    <Button variant="ghost" size="sm" class="h-6 text-[10px]" onclick={() => hiddenTypes.clear()}>
                        Reset
                    </Button>
                {/if}
            </div>
             ...
             {#each timeRanges as range, i}
                <div class="bg-secondary/30 p-4 rounded-lg space-y-4 relative group">
                    {#if timeRanges.length > 1}
                        <button onclick={() => removeTimeRange(i)} class="absolute top-2 right-2 text-muted-foreground hover:text-destructive p-1">
                            <X class="w-3 h-3" />
                        </button>
                    {/if}
                    ...
                    <Slider 
                        ...
                        min={filterData.bounds.min} max={filterData.bounds.max} step={15}
