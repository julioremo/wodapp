...
export function calculateOpenTime(
  ...
  } else if (policy.type === "fixed_day" && policy.dayOfWeek != null && policy.hour != null) {
...
export type AvailabilityState =
  ...
  | { type: "this_week"; dayStr: string; timeStr: string }
  | { type: "future"; dateStr: string; timeStr: string };

export function getAvailability(openTime: Date | string, now: Date): AvailabilityState {
  ...
  if (differenceInCalendarDays(target, now) < 7) {
    return {
      type: "this_week",
  ...
  return {
    type: "future",
    dateStr: format(target, "EEEE, d MMM"),
    timeStr
  };
}
