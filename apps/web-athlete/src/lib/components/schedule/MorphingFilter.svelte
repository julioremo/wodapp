  ...
  <div class="relative w-full h-full min-h-[40px] flex flex-col">
    <button
      ...
      <div class="relative w-5 h-5">
        <div
          class={cn("absolute inset-0 transition-opacity duration-300", isOpen ? "opacity-0" : "opacity-100")}>
          <Filter class="w-5 h-5 text-muted-foreground" />
        </div>
        <div
          class={cn("absolute inset-0 transition-opacity duration-300", isOpen ? "opacity-100" : "opacity-0")}>
    ...
    <div
      ...
      <span class="text-xs font-mono uppercase tracking-widest text-muted-foreground mb-1"
        >Filter by Class Type</span
      >
